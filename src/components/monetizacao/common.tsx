...
export function FalhaDeCarga({ data }: { data: BaseMonetizacao }) {
  ...
  return (
    <Notice>
      <p>
        <strong>Os indicadores comerciais estão parados{desde ? ` desde ${desde}` : ""}.</strong>{" "}
        A última tentativa falhou porque <span title={data.sync_error}>{motivoLegivel(data.sync_error)}</span>. Os
        números de reuniões, oportunidades e contratos abaixo são dessa última carga concluída, não
        de agora.
      </p>
      {catalogoOk && (
        <p className="mt-1">
    ...
    <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
      <span
        className={`h-2 w-2 rounded-full ${
          stale ? "bg-amber-500" : metricasVelhas || catalogoVelho ? "bg-sky-500" : "bg-emerald-500"
        }`}
      />
