# Plano · Receita e Repasses (módulo 4) no DS v2

Branch `feat/ds-v2-migracao-receita-20260924` (worktree `planning-brain-ds-v2-receita`), base `origin/main` `b5c44d7` + correção V4 do IDU (cherry-pick de `1bb0341`).
Contratos: `docs/design/contratos/receita-e-repasses.md`. Aprovação: Pedro, 24/09 ("pode seguir conforme suas propostas… é pra padronizar e melhorar TODAS as telas de TODOS os módulos"). Dono: Eliezek (revisa no PR).
...
| RR4 | Regras da Rede |
| RR5 | Apuração de Royalties (lista e ficha) |
| RR6 | Funil de CAC e Split do Asaas |
| RR7 | Comissões e EBIT Operacional |
| RR8 | Revisão final |
