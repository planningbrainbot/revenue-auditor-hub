...
language plpgsql security definer set search_path=ops,public,extensions as $$
declare c jsonb; units integer[]; e ops.monetizacao_envios;
begin
 ...
 delete from ops.monetizacao_deals where not exists(select 1 from jsonb_array_elements(_snapshot->'cards') c where (c->>'id')::bigint=id);
