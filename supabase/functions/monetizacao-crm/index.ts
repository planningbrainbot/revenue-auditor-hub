...
const headers = {
  ...
  "Access-Control-Allow-Origin": "https://planningbrain.com.br",
  "Access-Control-Allow-Headers": "authorization, apikey, content-type, x-client-info",
  "Access-Control-Allow-Methods": "POST, OPTIONS",
  "Cache-Control": "no-store",
};
function result(body: Row, status = 200) {
...
const { pd, pages, copyFile } = pipedriveApi(PD);
async function completeHandoff(nonce: string) {
  const ctx = await rpc("monetizacao_handoff_claim", { _nonce: nonce });
  if (!ctx)
    return (
      (await db(`monetizacao_envios?id=eq.${nonce}&select=handoff`, ADMIN))[0]?.handoff || {
        status: "pending",
      }
    );
  let report;
  try {
    report = await fillHandoff(ctx, { pd, pages, copyFile });
  } catch (e) {
    report = {
      status: "partial",
      errors: [e instanceof Error ? e.message : "Preenchimento complementar pendente"],
      warnings: [],
    };
  }
  await rpc("monetizacao_handoff_finish", { _nonce: nonce, _lease: ctx.lease, _result: report });
  return report;
}
...
async function collect() {
  ...
  const deals = [
    ...new Map(
      (
        await limited(stageRows, (s) =>
          pages("deals", { stage_id: s.id, status: "all_not_deleted" }),
        )
      )
        .flat()
        .filter((d) => d.pipeline_id === PIPE)
        .map((d) => [d.id, d]),
    ).values(),
  ];
  const summary = await Promise.all(
    ["open", "won", "lost"].map((status) => pd("deals/summary", { pipeline_id: PIPE, status })),
  );
  ...
  const signature = (rows: Row[], live = false) =>
    JSON.stringify(
      rows
        .filter((s) => !/(reciclad|perdid|descart|estacion|parking)/i.test(s.name))
        .map((s) => [s.id, s.name, live ? s.order_nr : s.order]),
    );
  ...
  const changed = deals.filter((d) => {
    ...
    return (
      ...
      !old.history_refreshed_at ||
      now - Date.parse(old.history_refreshed_at) > 86400000
    );
  });
  const flows = Object.fromEntries(
    await limited(changed, async (d) => [
      d.id,
      await pages(`deals/${d.id}/flow`, { items: "dealChange" }),
    ]),
  );
  ...
  snapshot.cards = snapshot.cards.map((c: Row) => {
    ...
    const flags = Object.fromEntries(
      Object.entries(old.events).map(([key, rows]) => [
        key,
        (rows as Row[]).some((e) => e.date.startsWith(month)),
      ]),
    );
    return { ...old, ...flags };
  });
...
async function sync() {
  ...
  try {
    ...
    const pending: Row[] = await db(
      "monetizacao_envios?select=id,item_id,status,org_id,product&status=in.(sending,uncertain)&order=created_at&limit=1000",
      ADMIN,
    );
    for (const e of pending) {
      const found = deals.filter((d) => String(d.title).includes(`[AQ:${e.id}]`));
      if (found.length === 1 && hasCanonicalProduct(found[0], e.product))
        await rpc("monetizacao_finish", {
          _nonce: e.id,
          _status: "sent",
          _deal: found[0].id,
          _org: id(found[0].org_id),
          _reason: "Envio conciliado pelo identificador único e pelo produto no CRM.",
        });
    }
    // Retoma apenas pacotes iniciados por um envio explícito, sem criar outro negócio.
    const incomplete: Row[] = await db(
      "monetizacao_envios?status=eq.sent&handoff=not.is.null&handoff->>status=neq.complete&select=id&order=updated_at&limit=1",
      ADMIN,
    );
    for (const e of incomplete) await completeHandoff(e.id).catch(() => {});
    ...
    await db(
      `monetizacao_sync?id=eq.true&run_id=eq.${run}`,
      ADMIN,
      { status: "error", error: message.slice(0, 240) },
      "PATCH",
    ).catch(() => {});
    throw e;
  }
...
async function send(itemIds: string[], token: string) {
  ...
  for (const item of itemIds) {
    let claim;
    try {
      claim = await rpc("monetizacao_claim", { _item: item }, token);
    } catch (e) {
      results.push({
        item,
        status: "blocked",
        reason: e instanceof Error ? e.message : "Não foi possível conferir esta oportunidade",
      });
      continue;
    }
    if (!claim.claimed) {
      if (claim.status === "sent") {
        const rows = await db(
          `monetizacao_envios?item_id=eq.${item}&status=eq.sent&select=id&limit=1`,
          ADMIN,
        );
        const handoff = rows[0]
          ? await completeHandoff(rows[0].id).catch(() => ({ status: "partial" }))
          : { status: "pending" };
        results.push({ item, ...claim, handoff });
      } else results.push({ item, ...claim });
      continue;
    }
    let remoteStarted = false,
      org: number | null = null,
      remoteDeal: number | null = null;
    const finish = async (status: string, deal: number | null, reason: string) => {
      await rpc("monetizacao_finish", {
        _nonce: claim.nonce,
        _status: status,
        _deal: deal,
      ...
      const handoff =
        status === "sent"
          ? await completeHandoff(claim.nonce).catch(() => ({
              status: "partial",
              errors: ["Card criado. Preenchimento complementar pendente; completar sem reenviar."],
            }))
          : undefined;
      results.push({ item, status, deal_id: deal, reason, handoff });
    };
    try {
      if (!users.some((u) => u.id === claim.owner_id && u.active_flag))
        throw new Error("Hunter não está ativo no Pipedrive");
      ...
      if (account.pipedrive_contract_id) {
        ...
        if (product === "finance" && contract?.status !== "won")
          throw new Error("O contrato de origem não está ganho no Pipedrive; revise o cadastro.");
      ...
      if (
        !hasCanonicalProduct(verified, product) ||
        id(verified.user_id) !== Number(claim.owner_id) ||
        id(verified.org_id) !== org
      ) {
        await finish(
          "uncertain",
          created.data.id,
          "Negócio criado, mas o produto não foi confirmado no CRM. Confira Caixa · Produto antes de qualquer reenvio.",
        );
        continue;
      }
      await finish(
        "sent",
        created.data.id,
        "Criada na etapa de entrada, com Caixa · Produto conferido no Pipedrive.",
      );
    } catch (e) {
      const message = e instanceof Error ? e.message : "Não foi possível confirmar o envio";
      const definite =
        !remoteStarted ||
        (!remoteDeal && /^Pipedrive HTTP (400|401|403|404|422|429)$/.test(message));
      await finish(
        definite ? "blocked" : "uncertain",
        remoteDeal,
        definite ? message : "Resposta incerta. Atualize para conciliar antes de qualquer reenvio.",
      ).catch(() => {
        results.push({
          item,
          status: "uncertain",
          reason: "Não foi possível confirmar o registro. Atualize para conciliar.",
        });
      });
    }
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const auth = await fetch(`${URL_BASE}/auth/v1/user`, {
      headers: { apikey: ANON, Authorization: `Bearer ${token}` },
    });
    if (!auth.ok) return result({ error: "Sessão inválida" }, 401);
    const key = body.action === "send" ? "send.monetizacao" : "view.monetizacao";
    if (!(await rpc("monetizacao_can", { _key: key }, token)))
      return result({ error: "Sem permissão para esta operação" }, 403);
    if (body.action === "sync") {
      if (!(await rpc("monetizacao_scope", { _ids: [] }, token)))
        return result({ error: "Atualização global restrita à equipe central" }, 403);
    ...
    if (
      ...
      body.items.every((x: unknown) => typeof x === "string" && /^[0-9a-f-]{36}$/.test(x))
    )
      return result(await send(body.items, token));
    return result({ error: "Ação ou seleção inválida" }, 400);
  } catch (e) {
    return result({ error: e instanceof Error ? e.message : "Falha na integração" }, 400);
  }
});
