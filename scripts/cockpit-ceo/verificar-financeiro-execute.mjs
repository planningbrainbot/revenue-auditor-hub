...
const [f] =
  await ro(`select count(*)::int funcoes, count(*) filter (where has_function_privilege('authenticated', p.oid, 'EXECUTE'))::int abertas_authenticated
  from pg_proc p join pg_namespace n on n.oid = p.pronamespace where n.nspname = 'financeiro'`);
const [v] = await ro(
  `select coalesce(array_to_string(reloptions, ','), '') opcoes from pg_class where oid = 'ops.qb_clientes_ativos'::regclass`,
);
const [socio] = await ro(
  `select user_id::text id from ops.user_roles where role = 'socio_regional' order by user_id limit 1`,
);
...
if (socio?.id && /^[0-9a-f-]{36}$/.test(socio.id)) {
  ...
  chamada = await ro(
    `${pre} select jsonb_array_length(financeiro.fn_faturamento_mensal(p_comp_de => '2026-01-01', p_comp_ate => '2026-08-01', p_limite_clientes => 1)->'serie') meses`,
  )
    .then((r) => `DEVOLVEU ${r[0].meses} meses (furo aberto)`)
    .catch((e) =>
      /permission denied/.test(e.message) ? "permission denied (fechado)" : e.message.slice(0, 120),
    );
  [view] = await ro(
    `${pre} select (select count(*) from ops.qb_clientes_ativos)::int pela_view, (select count(*) from ops.empresas)::int pela_rls`,
  );
}
console.log(
  JSON.stringify(
    { financeiro: f, socio_chama_fn_faturamento: chamada, qb_clientes_ativos: { ...v, ...view } },
    null,
    2,
  ),
);
