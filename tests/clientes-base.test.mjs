...
import { normalizeCnpj, resolveUnit, classifyOrigin, refinement, strictDate, canonicalContacts } from '../supabase/functions/base-clientes-sync/domain.mjs';
import { webhookRecord, mapCompany, mapContact } from '../supabase/functions/base-clientes-sync/pipefy.mjs';

const cnpj = '11222333000181'; // Documento sintético com dígitos verificadores válidos.
const units = [{ id: 1, pipefy_id: 'pf-rj', nome_da_praca: 'Rio de Janeiro' }, { id: 2, pipefy_id: 'pf-cwb', nome_da_praca: 'Curitiba' }, { id: 3, pipefy_id: 'pf-sbc', nome_da_praca: 'São Bernardo' }];
...
test('AC02/08: normaliza documento, recusa CPF, repetição e dígito verificador inválido', () => {
  ...
  for (const value of [null, '', '11111111111111', '12345678901', '11222333000182']) assert.equal(normalizeCnpj(value), null);
});
...
test('AC03: Omie fora de Curitiba é Nova, com ou sem pagamento, sobrepondo origem antiga declarada', () => {
  for (const payment of [null, false, true]) {
    const result = classifyOrigin({ omieUnits: ['Rio de Janeiro'], declared: 'Base Antiga', payment });
...
test('AC07/08: refinamento cumulativo exige ECD do CNPJ correto e não conta contato/negócio como empresa', () => {
  const rows = [
    { key: 'a', cnpjs: [cnpj], contacts: [{ type: 'email', value: 'a@example.test' }], ecd: [{ cnpj, year: 2024 }] },
  ...
  assert.equal(refinement([{ ...rows[0], ecd: [{ cnpj: '00000000000000', year: 2024 }] }]).counts.ecd, 0);
});
test('AC02: contato mantém identidade própria; e-mail/telefone vazios não comprovam contatabilidade', () => {
  assert.equal(canonicalContacts([{ id: '1', email: 'A@example.test' }, { id: '1', email: 'A@example.test' }]).length, 1);
...
test('AC10: recebe o formato atual de webhook Pipefy de tabela e não confia no valor enviado', () => {
  assert.deepEqual(webhookRecord({ data: { action: 'card.field_update', card: { id: 123, pipe_id: 'companies' }, new_value: 'não confiar' } }, ['companies']), { action: 'card.field_update', id: '123', table: 'companies', deleted: false });
  assert.equal(webhookRecord({ data: { action: 'card.create', card: { id: 123, pipe_id: 'outro-projeto' } } }, ['companies']), null);
  assert.equal(webhookRecord({ action: 'table_record.update', data: { table_record: { id: 123, table_id: 'companies' } } }, ['companies']).id, '123');
});
test('AC10/11: campo esvaziado na fonte vira nulo; campo que não existe no schema é preservado', () => {
  const record = { id: 'pf-company', title: 'Empresa sintética', updated_at: '2026-09-16T12:00:00Z', record_fields: [{ field: { id: 'cnpj' }, value: '11.222.333/0001-81' }, { field: { id: 'unidade' }, array_value: ['pf-rj'] }] };
...
test('AC02/10: vínculo de contato usa ID da empresa e deixa múltiplos vínculos explícitos', () => {
  const r = mapContact({ id: 'p1', title: 'Pessoa sintética', record_fields: [{ field: { id: 'empresa' }, array_value: ['e1', 'e2'] }, { field: { id: 'e_mail_direto' }, value: 'p@example.test' }] }, ['empresa', 'e_mail_direto', 'nome_completo']);
