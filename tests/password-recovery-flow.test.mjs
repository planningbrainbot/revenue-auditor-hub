...
test("Código funciona sem o link e somente como OTP de recuperação", async () => {
  ...
  assert.deepEqual(
    await flow.save("valid-password", { email: " synthetic@example.com ", token: "123456" }),
    { ok: true },
  );
  assert.deepEqual(calls[0], [
    "verify",
    { email: "synthetic@example.com", token: "123456", type: "recovery" },
  ]);
});

test("Link legado só estabelece sessão depois da confirmação", async () => {
  const { calls, auth } = fixture();
  const flow = createPasswordRecoveryFlow(auth, {
    kind: "legacy",
...
test("Link ausente não aproveita uma sessão já logada", async () => {
  const { calls, auth } = fixture();
  assert.equal(
    (await createPasswordRecoveryFlow(auth, { kind: "missing" }).save("valid-password")).ok,
    false,
  );
  assert.equal(calls.length, 0);
});
