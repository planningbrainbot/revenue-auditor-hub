...
test("carrega lotes com até quatro consultas em paralelo e preserva a ordem do manifesto", async () => {
  let running = 0,
    max = 0;
  const result = await loadCatalogPages(
    pages,
    async (page) => {
      running++;
      max = Math.max(max, running);
      await delay((Number(page.through) % 4) + 1);
      running--;
...
test("recusa duplicação entre lotes sem publicar total parcial", async () => {
  const boundaries = [
    { after: null, through: "b", count: 2 },
    { after: "b", through: "c", count: 2 },
  ];
  await assert.rejects(
    loadCatalogPages(
      boundaries,
      async (p) => ({
        ...version,
        accounts: (p.after ? ["b", "c"] : ["a", "b"]).map((key) => ({ key })),
      }),
      { ...version, base_count: 4 },
    ),
    /total parcial/,
  );
});
test("cancelamento encerra a carga e carteira vazia não faz consultas", async () => {
  ...
  await assert.rejects(
    loadCatalogPages(
      ...
      { ...version, base_count: 20, signal: controller.signal },
    ),
    { name: "AbortError" },
  );
  assert.equal(called, 0);
...
test("falha não inicia novos lotes e manifesto incompleto falha antes da rede", async () => {
  ...
  await assert.rejects(
    loadCatalogPages(
      ...
      async (p) => {
        called++;
        if (p.through === "0") throw new Error("offline");
        await delay(3);
        return { ...version, accounts: [{ key: p.through }] };
      },
      { ...version, base_count: 20 },
    ),
    /offline/,
