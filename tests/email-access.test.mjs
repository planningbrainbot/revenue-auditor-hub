...
const input = { to: 'usuario@example.com', subject: 'Acesso', html: '<p>Link de acesso</p>', text: 'Link de acesso' };
...
test('Acesso usa somente o remetente da Planning e preserva o destinatário solicitado', async (t) => {
  ...
  t.after(() => previous === undefined ? delete process.env.RESEND_API_KEY : process.env.RESEND_API_KEY = previous);
  const calls = [];
  ...
  assert.deepEqual(JSON.parse(calls[0].options.body), { from: 'Planning Brain <noreply@planningbrain.com.br>', to: [input.to], subject: input.subject, html: input.html, text: input.text });
...
test('Sem chave não chama nenhum provedor nem informa sucesso', async (t) => {
  ...
  t.after(() => previous === undefined ? delete process.env.RESEND_API_KEY : process.env.RESEND_API_KEY = previous);
  const fetch = t.mock.method(globalThis, 'fetch', () => { throw new Error('Não deveria enviar'); });
...
test('Erros do provedor e de rede não vazam o corpo da mensagem ou a chave', async (t) => {
  ...
  const fetch = t.mock.method(globalThis, 'fetch', async () => new Response('synthetic-secret', { status: 403 }));
  const rejected = await enviarEmailAcesso(input);
  assert.equal(rejected.enviado, false);
  fetch.mock.mockImplementation(async () => { throw new Error('synthetic-secret'); });
