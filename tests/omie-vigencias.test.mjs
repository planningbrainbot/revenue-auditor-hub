...
test('espaços de IDs por aplicativo e presença explícita do Simples', () => {
  const one = contractEvidence('Planning CWB 01', contracts, [{ ...client, optante_simples_nacional: 'N' }], '2026-09-17');
  const two = contractEvidence('Planning CWB 02', contracts, [{ ...client, optante_simples_nacional: 'S' }], '2026-09-17');
...
test('data impossível e CPF não viram vigência ou CNPJ válido', () => {
  const result = contractEvidence('Curitiba', [{ cabecalho: { ...contracts[0].cabecalho, dVigInicial: '31/02/2025' } }], [{ ...client, cnpj_cpf: '12345678901' }], '2026-09-17');
