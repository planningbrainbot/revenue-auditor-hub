...
test("Ler o link várias vezes não consome o token e não chama o Auth", (t) => {
  const fetch = t.mock.method(globalThis, "fetch", () => {
    throw new Error("Não chamar Auth ao abrir");
  });
  for (let i = 0; i < 5; i++)
    assert.equal(readRecoveryLink(passwordRecoveryLink(token)).kind, "token");
...
test("Tokens de outro fluxo e erros não autorizam redefinir a senha da sessão existente", () => {
  ...
  assert.equal(
    readRecoveryLink(`https://planningbrain.com.br/redefinir-senha#token_hash=${token}&type=invite`)
      .kind,
    "invalid",
  );
  assert.equal(
    readRecoveryLink(
      "https://planningbrain.com.br/redefinir-senha#error=access_denied&error_code=otp_expired",
    ).kind,
    "invalid",
  );
  assert.equal(
    readRecoveryLink(
      "https://planningbrain.com.br/redefinir-senha#access_token=ordinary-session&type=signin",
    ).kind,
    "missing",
  );
...
test("Emails antigos válidos preservam a identidade do token para conferir a sessão", () => {
  assert.deepEqual(
    readRecoveryLink(
      "https://planningbrain.com.br/redefinir-senha#access_token=legacy-token&refresh_token=refresh-token&type=recovery",
    ),
    { kind: "legacy", accessToken: "legacy-token", refreshToken: "refresh-token" },
  );
});
...
test("Fragmentos encaminhados com entidades HTML ou codificação continuam válidos", () => {
  ...
  assert.deepEqual(
    readRecoveryLink(passwordRecoveryLink(token).replace("&type", "&amp;type")),
    expected,
  );
  assert.deepEqual(
    readRecoveryLink(
      "https://planningbrain.com.br/redefinir-senha#" +
        encodeURIComponent(`token_hash=${token}&type=recovery`),
    ),
    expected,
  );
  assert.deepEqual(
    readRecoveryLink(
      `https://planningbrain.com.br/redefinir-senha?token_hash=${token}&type=recovery`,
    ),
    expected,
  );
  assert.equal(readRecoveryLink("malformed").kind, "invalid");
  assert.equal(
    readRecoveryLink("https://planningbrain.com.br/redefinir-senha#error_code=otp_expired").kind,
    "invalid",
  );
});
