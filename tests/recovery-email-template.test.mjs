...
test('Template publicado no Auth corresponde ao layout compartilhado, com link e código', () => {
  const html = emailRecuperacaoSenha({link:'https://planningbrain.com.br/redefinir-senha#token_hash={{ .TokenHash }}&type=recovery',codigo:'{{ .Token }}'}).html;
  assert.equal(readFileSync(new URL('../supabase/templates/recovery.html', import.meta.url),'utf8'),html+'\n');
